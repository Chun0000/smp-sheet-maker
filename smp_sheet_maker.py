...
def get_sample_info(path, patient, normal, out_file):
    """
        ...
        for file in files:
            try:
                ...
                if sample not in data:
                    data[sample] = {'status': 0 if sample == normal else 1, 'lane': lane, 'fastq_1': '', 'fastq_2': ''}
    ...
    df = pd.DataFrame.from_dict(data, orient='index').reset_index().rename(columns={'index': 'sample'})
    df['patient'] = patient
    df = df[['patient', 'status', 'sample', 'lane', 'fastq_1', 'fastq_2']]
    df.to_csv(out_file, index=False)

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Get sample information and save to a CSV file.")
    ...
    get_sample_info(args.path, args.patient, args.normal, args.out_file)